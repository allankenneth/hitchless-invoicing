...
My mobile invoicing system written in Python against Google App Engine and the Google Calendar API. This is a learning project for me, and it's not complete, though it is close to useable. It was written to scratch an itch and provide myself with a simple web-based invoicing system that logged time and create invoices from it. While it is quite flexible with minor tweaking, it is focussed on the single-person freelancer workflow/use-case.
...
It was developed with a "mobile-first" mindset and uses the jQuery Mobile framework. 
